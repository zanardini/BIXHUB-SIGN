...
internal class CustomJsonCodec
{
    ...
    {
        if (obj is not null and AbstractOpenAPISchema)
    ...
    {
        T result = (T)await Deserialize(response, typeof(T)).ConfigureAwait(false);
        return result;
    }
    ...
    internal async Task<object> Deserialize(HttpResponseMessage response, Type type)
    {
        ...
        foreach (KeyValuePair<string, IEnumerable<string>> responseHeader in response.Content.Headers)
            headers.Add(responseHeader.Key + "=" + ClientUtils.ParameterToString(responseHeader.Value));

        // RFC 2183 & RFC 2616
        Regex fileNameRegex = new(@"Content-Disposition=.*filename=['""]?([^'""\s]+)['""]?$", RegexOptions.IgnoreCase);
        ...
        else if (type == typeof(FileParameter))
        {
            if (headers != null)
                ...
                {
                    Match match = fileNameRegex.Match(header.ToString());
                    if (match.Success)
                    {
        ...
        if (type == typeof(Stream))
        {
            ...
            if (headers != null)
            {
                string filePath = string.IsNullOrEmpty(_configuration.TempFolderPath)
                    ? Path.GetTempPath()
        ...
        if (type.Name.StartsWith("System.Nullable`1[[System.DateTime")) // return a datetime object
            return DateTime.Parse(await response.Content.ReadAsStringAsync().ConfigureAwait(false), null, DateTimeStyles.RoundtripKind);
...
public partial class ApiClient : IDisposable, ISynchronousClient, IAsynchronousClient
{
    ...
    HttpContent PrepareMultipartFormDataContent(RequestOptions options)
    {
        ...
        if (options.FileParameters != null && options.FileParameters.Count > 0)
            foreach (KeyValuePair<string, IList<FileParameter>> fileParam in options.FileParameters)
                foreach (FileParameter file in fileParam.Value)
                {
                    StreamContent content = new(file.Content);
        ...
        IReadableConfiguration configuration)
    {
        ...
        if (options.HeaderParameters != null)
            foreach (KeyValuePair<string, IList<string>> headerParam in options.HeaderParameters)
                foreach (string value in headerParam.Value)
        ...
        if (contentType == "multipart/form-data")
            ...
            if (options.Data is FileParameter fp)
            {
                ...
                StreamContent streamContent = new(fp.Content);
                streamContent.Headers.ContentType = new MediaTypeHeaderValue(contentType);
                request.Content = streamContent;
            }
            else
            {
                CustomJsonCodec serializer = new(SerializerSettings, configuration);
    ...
    private async Task<ApiResponse<T>> ToApiResponse<T>(HttpResponseMessage response, T? responseData, Uri? uri)
    {
        T? result = responseData;
        string rawContent = await response.Content.ReadAsStringAsync().ConfigureAwait(false);

        ApiResponse<T> transformed = new(response.StatusCode, [], result, rawContent)
        {
        ...
        if (_httpClientHandler != null && response != null && uri is not null)
            try
            {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        CustomJsonCodec deserializer = new(SerializerSettings, configuration);
        ...
        try
        {
            ...
            List<Cookie>? cookieContainer = req.Properties.ContainsKey("CookieContainer") ? req.Properties["CookieContainer"] as List<Cookie> : null;
            ...
            {
                AsyncPolicy<HttpResponseMessage> policy = RetryConfiguration.AsyncRetryPolicy;
            ...
            T? responseData = await deserializer.Deserialize<T>(response).ConfigureAwait(false);

            // if the response type is oneOf/anyOf, call FromJSON to deserialize the data
            if (typeof(AbstractOpenAPISchema).IsAssignableFrom(typeof(T)))
            {
                Type type = typeof(T);
                MethodInfo? methodInfo = type.GetMethod("FromJson");
                if(methodInfo is not null)
                {
                    responseData = (T?)methodInfo.Invoke(null, [response.Content]);
                }
            }
        ...
        {
            if(ex is OperationCanceledException original)
            {
                if (timeoutTokenSource != null && timeoutTokenSource.IsCancellationRequested)
                    throw new TaskCanceledException($"[{req.Method}] {req.RequestUri} was timeout.",
                        new TimeoutException(original.GetMessage(), original));
            }
            throw new Exception($"Error Calling {req.RequestUri}", ex);
        }
        finally
        {
